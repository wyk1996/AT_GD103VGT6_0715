...
{
    rt_pin_mode(CH_ENABLE_PIN2, GPIO_MODE_OUT_PP);
    rt_pin_mode(CH_ENABLE_PIN1, GPIO_MODE_OUT_PP);
}
...
uint8_t ch_is_cp_off(CH_TASK_T *pstChTcb)
{
    ...
    curtick = rt_tick_get();
    if((CP_6V == pstChTcb->stGunStat.ucCpStat) || (CP_9V == pstChTcb->stGunStat.ucCpStat))    /* 6V和9V cp未断开,仅有12V断开 */
    {
        lasttick = curtick;
...
//    pstChTcb->stChargingData.usVout = pstChTcb->stRn8209.usVolt;
//    pstChTcb->stChargingData.usIout = pstChTcb->stRn8209.usCurrent / 10;
...
uint8_t ch_insert_proc(CH_TASK_T *pstChTcb)
{
    ...
    if(ch_is_start(pstChTcb))                               /* 开始充电，里面判断条件 */
    {
...
uint8_t ch_is_low_current(CH_TASK_T *pstChTcb)
{
    ...
    if(rt_tick_get() - pstChTcb->stLowCurr.uiTick <= (CH_TIME_60_SEC * 5))
    {
        return 0;
    }
...
uint8_t ch_is_stp(CH_TASK_T *pstChTcb)
{
    ...
    /* 大致为1s中时间 */
    if(((PRECHARGE == pstChTcb->ucState )  ||  (CHARGING == pstChTcb->ucState ) )/* 预充 */
...
//            && ch_is_low_current(pstChTcb))                                                   /* 持续无电流关机 amos */
    ...
    if(money_not_enough(pstChTcb))  /*单机收费卡 单机余额不足*/
    {
        return E_END_APP_BALANC;
    }
    ...
    if(ch_is_end_condition(pstChTcb))  /*预约模式下达到充电条件*/
    {
        return END_CONDITION;
    }

    if(ch_is_over_current(pstChTcb))   /* 过流 */
    {
        // stChRecord.ucStopReason = APP_OVER_CURRENT ;
        return  OVER_CURRENT;
    }

    if(ch_is_over_volt(pstChTcb))   /* 过压 */
    {
        //stChRecord.ucStopReason = APP_OVER_VOLT;
        return OVER_VOLT;
    }

    if(ch_is_under_volt(pstChTcb))   /* 欠压 */
    {
        //stChRecord.ucStopReason = APP_LOW_VOLT ;
        return UNDER_VOLT;
    }
